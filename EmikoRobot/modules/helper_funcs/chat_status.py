    ...
    def is_dev_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if not user:
            ...
            update.effective_message.reply_text(
                "Heihi chu Developer command bik ania."
                " Hmang ve turin thuneihna i neilo.",
            )
...
def sudo_plus(func):
    ...
    def is_sudo_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if not user:
            ...
            update.effective_message.reply_text(
                "Admin lo chuan thu min pe ve suh u? Hnar hnek in duhloh chuan😀",
            )
    ...
    def is_whitelist_plus_func(
        ...
        **kwargs,
        ...
        if user and is_whitelist_plus(chat, user.id):
            return func(update, context, *args, **kwargs)
        update.effective_message.reply_text(
            f"He thil hi i ti ve thei ngawt lo.\nTah hian lo kal teh @{SUPPORT_CHAT}",
        )
...
def bot_admin(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            not_admin = "Admin ka nilo tlat!"
        else:
            not_admin = f"<b>{update_chat_title}</b> ah hian Admin ka nilo!"
...
def bot_can_delete(func):
    ...
    def delete_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_delete = "Group ah hian message ka delete theilo!\nAdmin ah min dah phawt angai."
        else:
            cant_delete = f"Message te <b>{update_chat_title}</b> ah hian ka delete theilo!\nAdmin ah min dah phawt angai."
...
def can_pin(func):
    ...
    def pin_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_pin = (
                "Group ah hian message ka pin theilo!\nAdmin ah min dah phawt angai."
            )
        else:
            cant_pin = f"Message <b>{update_chat_title}</b> ah hian ka pin theilo!\nAdmin ah min dah phawt angai."
...
def can_promote(func):
    ...
    def promote_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_promote = "Members te ka promote/demote theilo!\nAdmin ah min dah phawt angai."
        else:
            cant_promote = (
                f"Members te ka promote/demote theilo<b>{update_chat_title}</b> ah hian!\n"
                f"Admin ah min dah phawt angai."
            )
...
def can_restrict(func):
    ...
    def restrict_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_restrict = "Member te ka thunun theilo!\nThunun tur chuan admin ah min dah phawt rawh."
        else:
            cant_restrict = f"<b>{update_chat_title}</b> a member te hi ka thunun theilo!\nThunun tur chuan admin ah min dah phawt rawh."
...
def connection_status(func):
    ...
    def connected_status(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update.effective_message.chat.type == "private":
            update.effective_message.reply_text(
                "/connect tih hi nang leh kei kan awmdun na Group ah thawn phawt rawh.",
            )
